<template>
  ...
  <CommonOverlayPopup :show="showCancelPopup">
    <OrdersCancel @close="showCancelPopup = false"  :orderSelected="orderSelected" :statusAllowedCancelOnly="statusAllowedCancelOnly"/>
  </CommonOverlayPopup>
  <!-- {{ order }} -->
  <div class="accordion-item border" v-if="order">

    <h3 class="accordion-header" :id="`heading-${order.id}`">
      <button class="accordion-button bg-transparent" :class="{ collapsed: index !== 0 }" type="button"
    ...
    <div :id="`collapse-${order.id}`" class="accordion-collapse collapse " :class="{ show: index === 0 }"
      :aria-labelledby="`heading-${order.id}`" data-bs-parent="#accordionAbout6">
          ...
          <div v-for="(item, index) in order.items" :key="index" class="order_boxes">
            ...
            <div class="full">
              ...
              <div class="product__details">
                ...
                <h6 v-if="item.variant_name" style="font-size: 0.875rem">{{ cleanedVariantName(item.variant_name) }}
                </h6>
                <!-- <h6>
            ...
            <div v-if="statusAllowedCancel.includes(order.order_status) && !isCancelRequested(item) && order.ordercancel == null"
              ...
              <div v-if="!showReason[order.id]?.[item.id]">
                ...
                <div class="d-flex justify-content-between align-items-center">
                  <span class="text-danger fw-semibold">
                    ...
                    {{ statusAllowedCancelOnly.includes(order.order_status)
              ...
              <div v-else>
                ...
                <textarea id="cancelReason" v-model="reasons[order.id][item.product_id]" rows="4"
                  class="form-control mb-3" placeholder="Type your reason here..."></textarea>

                <div class="d-flex justify-content-end">
                  <button class="btn btn-secondary me-2" @click="toggleReason(order.id, item.id)">Back</button>
                  <button class="btn btn-danger"
                    @click="submitCancellation(order.id, item.product_id, order.order_no, order)"
                    :disabled="!reasons[order.id]?.[item.product_id]?.trim()">
                    Submit Request
                  </button>
                </div>
              </div>
...
// Submit cancellation
async function submitCancellation(id, product_id, order_no, order) {
  const reason = reasons.value[id]?.[product_id]?.trim();

  const type = statusAllowedCancelOnly.includes(order.order_status)
    ? 'cancel'
    : 'return';
  ...
  if (reason) {
    ...
    showLoader();
    const payload = { order_no: order_no, reason: reason, product_id, type: type };
...
function openOrderCancel(order) {
